...
with open ("mergedfile_df_p2.csv", "w") as w1:
                ...
                if AMDID in line:
                    ...
                    for item in listofSNPs1:
                        if line.split(",")[3][6:8]!="1A":
                            ...
                            w1.write(line.strip("\n")+","+line.split(",")[3][6:8]+","+item+","+str(row[item])+"\n")
                        else:
                            w1.write(line.strip("\n")+",1"+","+item+","+str(row[item])+"\n")
                #for line2 in r2:
                    #if "CSID,AMD_ID,Sample_ID" not in 

with open ("mergedfile_df_p2.csv", "r") as w2:
    with open ("Gp1_pooled_merge_final_df_p2.csv", "w") as w3:        
        for line in w2:
            #print(line.split(",")[-2])
            #print(line.split(",")[-1])
            #print(line)
